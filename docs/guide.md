...
|------|------|
| **风格** | 偏工具 / 文档站：低饱和、少装饰 |
| **色彩** | 背景与表面 / 文本与边框 / 功能色三层；默认配色 `coral`（主色 `#9F5964`） |
| **比例** | 底与字约占 90%，主色与状态色各约 ≤5% |
| **字体** | 展示页 UI 用 Noto Sans SC；代码用 JetBrains Mono；字号 / 字重分层 |
| **圆角** | 使用 `--blora-radius-*`；控件默认全圆角，卡片用较大圆角 |
| **阴影** | 使用 `--blora-shadow-1…4`，浮层略抬起 |
| **动效** | 160–420ms，`cubic-bezier(.25,.8,.25,1)`；尊重 `prefers-reduced-motion` |
| **触控 / a11y** | 可点区域建议 ≥ 40×40；对比度按 WCAG AA |
...
> 目标：控件、反馈、表格、导航都用 Blora Design，而不是「按钮用我们的、下拉用浏览器原生、弹窗临时自写」。
...
- [ ] 字体与展示页接近（或接受令牌默认字体栈）  
- [ ] 主流程没有未包 class 的裸 input / select / button  
- [ ] 错误提示走 Blora Design（popup 或 validator），不用浏览器默认校验气泡  
- [ ] 对话框 / 抽屉 / 轻提示用 Blora Design，不用 `alert`  
- [ ] 颜色与间距来自令牌；暗色模式可切换且无局部漏改  
- [ ] 表格 / 分页 / 表单行为与展示页同类 demo 一致  
- [ ] 动态插入区域调用了 `Blora.init`  
- [ ] 未混用另一套 UI 库的控件皮肤  

全部勾选后，界面应与 `index.html` 属于同一套组件与令牌。
