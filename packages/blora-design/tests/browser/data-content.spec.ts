...
test("single-digit badges stay circular in default and avatar hosts", async ({ page }) => {
  const css = `${loadComponentCss("badge")}\n${loadComponentCss("avatar")}`;
  await page.setContent(
    pageWith(
      css,
      `
      <span class="blora-badge" id="plain">9</span>
      <div class="blora-avatar-wrap">
        <div class="blora-avatar">AB</div>
        <span class="blora-badge" id="on-avatar">3</span>
      </div>
      <span class="blora-badge" id="circle" data-variant="circle">3</span>
    `,
    ),
  );

  for (const id of ["plain", "on-avatar", "circle"]) {
    const box = await page.locator(`#${id}`).evaluate((el: HTMLElement) => {
